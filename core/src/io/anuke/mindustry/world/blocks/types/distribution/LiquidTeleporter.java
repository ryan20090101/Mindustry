...
public class LiquidTeleporter extends LiquidPowerBlock{
	...
	public LiquidTeleporter(String name) {
		super(name);
		update = true;
		solid = true;
		health = 80;
	...
	public void placed(Tile tile){
		tile.<LiquidTeleporterEntity>entity().channel = lastChannel;
		Timers.run(1f, () -> setConfigure(tile, lastChannel));
	}

	@Override
	public void configure(Tile tile, byte data) {
		LiquidTeleporterEntity entity = tile.entity();
		if(entity != null){
			entity.channel = data;
		}
	}
	...
	public void draw(Tile tile){
		LiquidTeleporterEntity ent = tile.entity();
	...
	public boolean acceptItem(Item item, Tile tile, Tile source){
		LiquidPowerEntity entity = tile.entity();
		return !(source.block() instanceof LiquidTeleporter) && entity.power >= powerPerItem && findLinks(tile).size > 0;
	}
	
	@Override
	public TileEntity getEntity(){
		return new LiquidTeleporterEntity();
	}
	...
	private Array<Tile> findLinks(Tile tile){
		...
		for(Tile other : teleporters[entity.channel]){
			if(other != tile){
				if(other.block() instanceof LiquidTeleporter){
					if(other.<LiquidTeleporterEntity>entity().channel != entity.channel){
						removal.add(other);
	...
	public static class LiquidTeleporterEntity extends LiquidPowerEntity{
