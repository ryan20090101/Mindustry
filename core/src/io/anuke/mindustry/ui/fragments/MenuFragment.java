		...
		new table(){{
			...
			if(!android){
				new table(){{
					...
					add(new MenuButton("$text.tutorial", group, ()-> control.playMap(global.maps().getMap("tutorial"))));
					row();
				...
				new table() {{
					...
					new imagebutton("icon-tutorial", isize, () -> control.playMap(global.maps().getMap("tutorial"))).text("$text.tutorial").padTop(4f);
