...
public class AltDimQuadTree<T> {
    ...
    private AltDimQuadTree<T> bottomLeftChild;
    private AltDimQuadTree<T> bottomRightChild;
    private AltDimQuadTree<T> topLeftChild;
    private AltDimQuadTree<T> topRightChild;
     ...
     */
    public AltDimQuadTree(int maxObjectsPerNode, Rectangle bounds) {
    ...
    private AltDimQuadTree(int maxObjectsPerNode, int level, Rectangle bounds, BoundingBoxProvider provider) {
        this.level = level;
        this.bounds = bounds;
    ...
    private void split() {
        ...
        leaf = false;
        bottomLeftChild = new AltDimQuadTree<T>(maxObjectsPerNode, level + 1, new Rectangle(bounds.x, bounds.y, subW, subH), provider);
        bottomRightChild = new AltDimQuadTree<T>(maxObjectsPerNode, level + 1, new Rectangle(bounds.x + subW, bounds.y, subW, subH), provider);
        topLeftChild = new AltDimQuadTree<T>(maxObjectsPerNode, level + 1, new Rectangle(bounds.x, bounds.y + subH, subW, subH), provider);
        topRightChild = new AltDimQuadTree<T>(maxObjectsPerNode, level + 1, new Rectangle(bounds.x + subW, bounds.y + subH, subW, subH), provider);
        ...
        for (Iterator<T> iterator = objects.iterator(); iterator.hasNext(); ) {
            ...
            AltDimQuadTree<T> child = getFittingChild(tmp);
            if (child != null) {
                child.insert(obj);
                iterator.remove();
            }
    ...
    private AltDimQuadTree<T> getFittingChild(Rectangle boundingBox) {
     ...
     */
    public AltDimQuadTree<T> getNodeAt(float x, float y) {
     ...
     */
    public AltDimQuadTree<T> getBottomLeftChild() {
        return bottomLeftChild;
    }
     ...
     */
    public AltDimQuadTree<T> getBottomRightChild() {
        return bottomRightChild;
    }
     ...
     */
    public AltDimQuadTree<T> getTopLeftChild() {
        return topLeftChild;
    }
     ...
     */
    public AltDimQuadTree<T> getTopRightChild() {
        return topRightChild;
    }
