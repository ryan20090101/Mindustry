...
import io.anuke.mindustry.util.AltDimQuadTree;
import io.anuke.ucore.util.QuadTree;

import java.util.Iterator;

public class AltDimEntityGroup<T extends AltDimEntity>{
    ...
    private AltDimQuadTree<SolidAltDimEntity> tree;
    private Class<T> type;

    public  final boolean useTree;

    public AltDimEntityGroup(Class<T> type, boolean useTree){
        this.useTree = useTree;
        this.id = lastid ++;
        this.type = type;
    }

    public AltDimEntityGroup<T> enableMapping(){
        map = new IntMap<>();
        return this;
    }
    ...
    public AltDimQuadTree<SolidAltDimEntity> tree(){
        return tree;
    }

    public void setTree(float x, float y, float w, float h){
        tree = new AltDimQuadTree(AltDimEntities.maxLeafObjects, new Rectangle(x, y, w, h));
    }
    ...
    public synchronized void clear(){
        for(AltDimEntity entity : entityArray)
            entity.group = null;

        for(AltDimEntity entity : entitiesToAdd)
            entity.group = null;

        for(AltDimEntity entity : entitiesToRemove)
            entity.group = null;
