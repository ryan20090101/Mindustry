...
public class AltDimEntityCollisions {
    ...
    public void move(SolidAltDimEntity entity, float deltax, float deltay){
    ...
    public void moveInternal(SolidAltDimEntity entity, float deltax, float deltay, boolean x){
        if(collider == null)
    ...
    public void updatePhysics(AltDimEntityGroup<?> group){
        collided.clear();

        QuadTree<SolidAltDimEntity> tree = group.tree();

        tree.clear();

        for(AltDimEntity entity : group.all()){
            if(entity instanceof SolidAltDimEntity){
                SolidAltDimEntity s = (SolidAltDimEntity)entity;
                s.lastX = s.x;
                s.lastY = s.y;
                tree.insert(s);
            }
    ...
    private void checkCollide(AltDimEntity entity, AltDimEntity other){
        SolidAltDimEntity a = (SolidAltDimEntity) entity;
        SolidAltDimEntity b = (SolidAltDimEntity) other;
    ...
    public void collideGroups(AltDimEntityGroup<?> groupa, AltDimEntityGroup<?> groupb){
        collided.clear();

        for(AltDimEntity entity : groupa.all()){
            if(!(entity instanceof SolidAltDimEntity))
                continue;
            if(collided.contains(entity.id))
                continue;

            SolidAltDimEntity solid = (SolidAltDimEntity)entity;

            solid.getBoundingBox(r2);
            solid.hitbox.getRect(r1, solid.lastX, solid.lastY);
            r2.merge(r1);

            synchronized (world[entity.dimension].ents.entityLock) {
